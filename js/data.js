...
const SKILLS = {
  warrior:[
    {name:'熊人變身', role:'ultimate', type:'transform', form:'bearman',  cd:12000, uses:3, dur:6000, amt:8,  frac:0.25, taunt:true, tier:3, icon:'shield', desc:'變身為熊人：限時 DEF 大增、受傷 −25%、嘲諷敵人'},
    {name:'牛頭人變身', role:'ultimate', type:'transform', form:'minotaur', cd:12000, uses:3, dur:6000, amt:12, frac:0.50, tier:3, icon:'axe', desc:'變身為牛頭人：限時 ATK 大增、破甲 50%、暴擊'},
    {name:'挑釁', role:'active', type:'taunt', cd:6000, uses:3, dur:2500, tier:2, icon:'target', desc:'數秒內敵人優先攻擊戰士'},
    {name:'大吼', role:'active', type:'stunAll', cd:8000, uses:2, dur:800, tier:2, icon:'stun', desc:'對全體敵人暈眩 0.8s'},
    {name:'蠻牛衝鋒', role:'active', type:'nuke', cd:9000, uses:3, mult:2.5, tier:3, icon:'smash', desc:'一次重擊，傷害 ×2.5'},
    {name:'連擊', role:'active', type:'doubleHit', cd:5000, uses:4, tier:2, icon:'arrows', desc:'攻擊時追加一擊'},
    {name:'戰吼', role:'active', type:'atkBuff', cd:8000, uses:3, amt:8, dur:4000, tier:1, icon:'flame', desc:'數秒內自身 ATK +8'},
    {name:'棘刺皮毛', role:'passive', type:'reflect', frac:0.25, tier:2, icon:'reflect', desc:'反彈受到傷害 25%'},
    {name:'不屈', role:'passive', type:'deathSave', tier:2, icon:'ward', desc:'每場首次致命傷免死，留 1 HP'},
    {name:'破甲', role:'passive', type:'pierce', frac:0.40, tier:3, icon:'dagger', desc:'攻擊無視 40% 敵方 DEF'},
  ...
  ranger:[
    ...
    {name:'爆裂箭', role:'active', type:'nuke', cd:7000, uses:3, mult:2, tier:2, icon:'fireball', desc:'一箭重擊 ×2'},
    {name:'疾風', role:'active', type:'atkBuff', cd:8000, uses:3, amt:6, dur:4000, tier:1, icon:'flame', desc:'數秒內自身 ATK +6'},
    {name:'鷹眼', role:'passive', type:'critVsFull', tier:2, icon:'eye', desc:'對滿血敵人必定暴擊'},
    {name:'致命', role:'passive', type:'critVsStunned', tier:2, icon:'skull', desc:'對暈眩敵人必定暴擊'},
    {name:'穿甲箭', role:'passive', type:'pierce', frac:0.40, tier:3, icon:'dagger', desc:'攻擊無視 40% 敵方 DEF'},
  ...
  priest:[
    ...
    {name:'聖光爆', role:'active', type:'nuke', cd:8000, uses:2, mult:2, tier:2, icon:'sparkle', desc:'聖光爆發 ×2'},
    {name:'鼓舞', role:'active', type:'atkBuff', cd:8000, uses:3, amt:6, dur:4000, tier:1, icon:'flame', desc:'數秒內自身 ATK +6'},
    {name:'聖療', role:'passive', type:'critHealLow', tier:2, icon:'heal', desc:'對半血以下對象治療翻倍'},
    {name:'聖盾', role:'passive', type:'shieldOnHeal', amt:12, tier:2, icon:'shield', desc:'被治療者獲得 12 護盾'},
    {name:'神恩', role:'passive', type:'cleanseOnHeal', tier:2, icon:'sparkle', desc:'治療同時解除暈眩'},
    {name:'聖佑', role:'passive', type:'deathSave', tier:2, icon:'ward', desc:'每場首次致命傷免死'},
    {name:'護心', role:'passive', type:'startShield', amt:15, tier:1, icon:'shield', desc:'每場開場護盾 +15'} ],
  mage:[
    {name:'靈魂震爆', role:'ultimate', type:'nuke', cd:10000, uses:3, mult:2.8, tier:3, icon:'sparkle', desc:'靈魂魔法，毀滅爆發 ×2.8'},
    {name:'隕石術', role:'ultimate', type:'stunAll', cd:11000, uses:2, dur:1200, tier:3, icon:'fireball', desc:'傷害魔法，隕石轟炸＋暈眩全體'},
    {name:'炎爆', role:'active', type:'crit', cd:5000, uses:2, mult:1.8, tier:2, icon:'fireball', desc:'法術暴擊 ×1.8'},
    {name:'連環施法', role:'active', type:'doubleHit', cd:5000, uses:2, tier:2, icon:'sparkle', desc:'追加一次施法'},
    {name:'火球', role:'active', type:'nuke', cd:7000, uses:3, mult:2.2, tier:2, icon:'fireball', desc:'火球重擊 ×2.2'},
    {name:'冰封', role:'active', type:'stun', cd:7000, uses:2, dur:1100, tier:2, icon:'stun', desc:'冰封暈眩 1.1s'},
    {name:'過載', role:'active', type:'atkBuff', cd:8000, uses:3, amt:8, dur:4000, tier:2, icon:'flame', desc:'數秒內自身 ATK +8'},
    {name:'奧術精通', role:'passive', type:'aoeBonus', tier:3, icon:'sparkle', desc:'範圍命中 2+ 時追加全體傷害'},
    {name:'護體罩', role:'passive', type:'deathSave', tier:2, icon:'ward', desc:'每場首次致命傷免死'},
    {name:'法力護盾', role:'passive', type:'startShield', amt:18, tier:1, icon:'shield', desc:'每場開場護盾 +18'},
    {name:'奧術穿透', role:'passive', type:'pierce', frac:0.40, tier:3, icon:'dagger', desc:'攻擊無視 40% 敵方 DEF'},
  ...
  rogue:[
    ...
    {name:'致命突襲', role:'active', type:'nuke', cd:7000, uses:3, mult:2.2, tier:2, icon:'smash', desc:'突襲重擊 ×2.2'},
    {name:'疾影', role:'active', type:'atkBuff', cd:8000, uses:3, amt:7, dur:4000, tier:1, icon:'flame', desc:'數秒內自身 ATK +7'},
    {name:'致命', role:'passive', type:'critVsStunned', tier:2, icon:'skull', desc:'對暈眩敵人必定暴擊'},
    {name:'淬毒', role:'passive', type:'pierce', frac:0.45, tier:3, icon:'dagger', desc:'淬毒鋒刃，攻擊無視 45% DEF'},
    {name:'嗜血', role:'passive', type:'lifesteal', frac:0.20, tier:2, icon:'heart', desc:'造成傷害回復 20% HP'},
    {name:'鋒利', role:'passive', type:'atkBonus', amt:6, tier:1, icon:'sword', desc:'ATK +6'},
    {name:'韌性', role:'passive', type:'lowHpDef', tier:1, icon:'shield', desc:'低血時 DEF 翻倍且免暈'} ],
};
